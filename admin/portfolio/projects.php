...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!csrfIsValid(isset($_POST['csrf_token']) ? (string) $_POST['csrf_token'] : null)) {
        ...
        $categoryName = '';
        if ($categoryId > 0) {
            $categoryStmt = $pdo->prepare('SELECT name FROM portfolio_categories WHERE id=:id');
        ...
        if ($categoryId <= 0 || $categoryName === '') $errors[] = 'Choose a valid category.';
...
$projects = $pdo->query('SELECT p.*, c.name AS category_name FROM portfolio_projects p JOIN portfolio_categories c ON c.id=p.category_id ORDER BY c.name ASC, p.display_order ASC, p.id DESC')->fetchAll();
$form = $edit ?: ['id'=>'','category_id'=>'','slug'=>'','short_description'=>'','full_description'=>'','featured_image'=>'','project_status'=>'Completed','project_location'=>'','client_name'=>'','completion_year'=>'','project_area'=>'','youtube_url'=>'','is_featured'=>0,'is_active'=>1,'display_order'=>count($projects)+1,'seo_title'=>'','seo_description'=>'','meta_keywords'=>'','og_image'=>''];
